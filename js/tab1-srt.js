...
// --- 模組級變數 ---
let currentVideoId = null; // 用於在獲取列表和下載字幕之間傳遞 videoId
...
function initializeTab1() {
    ...
    async function handleFetchYoutubeInfo() {
        ...
        youtubeLangSelectorContainer.classList.add('hidden');
        currentVideoId = getYouTubeVideoId(url); // Store video ID

        if (!currentVideoId) {
            showToast('無法從網址中解析出有效的 YouTube 影片 ID。', { type: 'error', duration: 5000 });
        ...
        try {
            ...
            tracks.forEach(track => {
                ...
                option.value = track.lang; // Use lang code as the value
    ...
    async function handleGetYoutubeSrt() {
        const selectedLang = youtubeLangSelect.value;
        if (!selectedLang) {
            showToast('請先選擇一個字幕語言。', { type: 'error' });
            return;
        }
        if (!currentVideoId) {
            showToast('發生內部錯誤：遺失 Video ID。', { type: 'error' });
            return;
        }
